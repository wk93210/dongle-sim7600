...
struct discovery_cache {
  struct {
    struct pdiscovery_cache_item *first;
    struct pdiscovery_cache_item *last;
    ast_rwlock_t lock;
  } items;
};
...
static const struct pdiscovery_device device_ids[] = {
    ...
    {0x12d1,
     0x1506,
     {1, 2, /* 0 */}}, /* E171 firmware 21.x : thanks Sergey Ivanov */
...
static int ports_match(const struct pdiscovery_ports *p1,
                       const struct pdiscovery_ports *p2) {
  unsigned i;
  for (i = 0; i < ITEMS_OF(p1->ports); i++) {
    if (!p1->ports[i] || !p2->ports[i] ||
        strcmp(p1->ports[i], p2->ports[i]) != 0)
      return 0;
  }
  return i;
}
...
#/* */
static int ports_copy(struct pdiscovery_ports *dst,
                      const struct pdiscovery_ports *src) {
  unsigned i;
...
#/* */
static void info_copy(struct pdiscovery_result *dst,
                      const struct pdiscovery_result *src) {
  if (src->imei)
...
#/* */
static void cache_item_update(struct pdiscovery_cache_item *item,
                              const struct pdiscovery_result *res, int status) {
  info_free(&item->res);
...
#/* */
static struct pdiscovery_cache_item *
cache_item_create(const struct pdiscovery_result *res, int status) {
...
#/* */
static struct pdiscovery_cache_item *
cache_search(struct discovery_cache *cache,
...
#/* */
static int cache_lookup(struct discovery_cache *cache,
                        const struct pdiscovery_request *req,
                        struct pdiscovery_result *res, int *failed) {
  int found = 0;
  ...
  if (item) {
    ...
    found = item->status ||
            ((req->imei || item->res.imei) && (req->imsi || item->res.imsi));
    if (found) {
      *failed = item->status;
    }
  }
...
#/* */
static void cache_update(struct discovery_cache *cache,
                         const struct pdiscovery_result *res, int status) {
  struct pdiscovery_cache_item *item = cache_search(cache, res);
  if (item) {
    cache_item_update(item, res, status);
...
#/* */
static const struct pdiscovery_cache_item *
cache_first_readlock(struct discovery_cache *cache) {
...
#/* */
static int pdiscovery_get_id(const char *name, int len, const char *filename,
                             unsigned *integer) {
  int len2;
  char *name2;
  int assign = 0;
...
#/* */
static char *pdiscovery_interface(const char *name, int len,
                                  unsigned *interface) {
  char *port = NULL;
...
#/* */
static char *pdiscovery_find_port(const char *name, int len, const char *subdir,
                                  unsigned *interface) {
  int len2;
  char *name2;
  struct stat statb;
...
#/* */
static int pdiscovery_interfaces(const char *devname, const char *name, int len,
...
#/* */
static const struct pdiscovery_device *
pdiscovery_lookup_ids(const char *devname, const char *name, int len) {
  unsigned vid;
  unsigned pid;
  unsigned idx;

  if (pdiscovery_get_id(name, len, "idVendor", &vid) == 1 &&
      pdiscovery_get_id(name, len, "idProduct", &pid) == 1) {
    ast_debug(4, "[%s discovery] found %s is idVendor %04x idProduct %04x\n",
              devname, name, vid, pid);
    for (idx = 0; idx < ITEMS_OF(device_ids); idx++) {
      if (device_ids[idx].vendor_id == vid &&
          device_ids[idx].product_id == pid) {
...
static int pdiscovery_handle_response(const struct pdiscovery_request *req,
                                      const struct iovec iov[2], int iovcnt,
                                      struct pdiscovery_result *res) {
  int done = 0;
  char *str;
  char sym;
...
static int pdiscovery_do_cmd(const struct pdiscovery_request *req, int fd,
                             const char *name, const char *cmd, unsigned length,
                             struct pdiscovery_result *res) {
  int timeout;
  char buf[1024 + 1];
  ...
  if (wrote == length) {
    ...
    ast_log(LOG_ERROR,
            "[%s discovery] failed to get valid response from %s in %d msec\n",
...
static int pdiscovery_get_info(const char *port,
                               const struct pdiscovery_request *req,
                               struct pdiscovery_result *res) {
  static const struct {
    const char *cmd;
  ...
  if (fd >= 0) {
    ...
    /* clean queue first ? */
    fail =
        pdiscovery_do_cmd(req, fd, port, cmds[cmd].cmd, cmds[cmd].length, res);
...
#/* return non-zero on fail */
static int pdiscovery_get_info_cached(const char *port,
                                      const struct pdiscovery_request *req,
                                      struct pdiscovery_result *res) {
  int fail = 1;
...
static int pdiscovery_read_info(const struct pdiscovery_request *req,
                                struct pdiscovery_result *res) {
...
#/* */
static int pdiscovery_check_req(const struct pdiscovery_request *req,
                                struct pdiscovery_result *res) {
  int match = 0;
...
#/* */
static int pdiscovery_check_device(const char *name, int len,
                                   const char *subdir,
                                   const struct pdiscovery_request *req,
                                   struct pdiscovery_result *res) {
  int len2;
  ...
  if (device) {
    ...
    if (res->ports.ports[INTERFACE_TYPE_DATA] &&
...
#/* */
static int pdiscovery_request_do(const char *name, int len,
                                 const struct pdiscovery_request *req,
                                 struct pdiscovery_result *res) {
  int found = 0;
  struct dirent *dentry;
  ...
  if (dir) {
    while ((dentry = readdir(dir)) != NULL) {
      if (strcmp(dentry->d_name, ".") != 0 &&
          strcmp(dentry->d_name, "..") != 0 &&
          strstr(dentry->d_name, "usb") != dentry->d_name) {
...
#/* */
EXPORT_DEF void pdiscovery_init() { cache_init(&cache); }

#/* */
EXPORT_DEF void pdiscovery_fini() { cache_fini(&cache); }

#/* */
EXPORT_DEF int pdiscovery_lookup(const char *devname, const char *imei,
                                 const char *imsi, char **dport, char **aport) {
  int found;
  struct pdiscovery_result res;
  const struct pdiscovery_request req = {
      devname,
  ...
  found = pdiscovery_request_do(sys_bus_usb_devices,
                                STRLEN(sys_bus_usb_devices), &req, &res);
  if (found) {
...
#/* */
EXPORT_DEF const struct pdiscovery_result *
pdiscovery_list_begin(const struct pdiscovery_cache_item **opaque) {
  ...
  memset(&res, 0, sizeof(res));
  pdiscovery_request_do(sys_bus_usb_devices, STRLEN(sys_bus_usb_devices), &req,
                        &res);
  result_free(&res);
...
#/* */
EXPORT_DECL const struct pdiscovery_result *
pdiscovery_list_next(const struct pdiscovery_cache_item **opaque) {
...
#/* */
EXPORT_DECL void pdiscovery_list_end() { cache_unlock(&cache); }
