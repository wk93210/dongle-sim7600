...
struct mixbuffer {
  struct {
    struct mixstream *first;
    struct mixstream *last;
  } streams;          /*!< list of stream descriptions */
...
INLINE_DECL size_t mixb_free(const struct mixbuffer *mb,
                             const struct mixstream *stream) {
...
EXPORT_DECL size_t mixb_write(struct mixbuffer *mb, struct mixstream *stream,
                              const char *data, size_t len);

/* get data pointer and sizes in iov for all available for reading data in
 * buffer */
INLINE_DECL int mixb_read_all_iov(const struct mixbuffer *mb,
                                  struct iovec iov[2]) {
  return rb_read_all_iov(&mb->rb, iov);
}

/* get data pointer and sizes in iov only for first len bytes */
INLINE_DECL int mixb_read_n_iov(const struct mixbuffer *mb, struct iovec iov[2],
                                size_t len) {
