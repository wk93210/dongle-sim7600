...
INLINE_DECL const char *dev_state2str_msg(dev_state_t state) {
  static const char *const states[] = {"Stop scheduled", "Restart scheduled",
                                       "Removal scheduled", "Start scheduled"};
...
typedef struct pvt_state {
  ...
  uint8_t
      chan_count[CALL_STATES_NUMBER]; /*!< channel number grouped by state */
} pvt_state_t;
...
typedef struct pvt_stat {
  ...
  uint32_t d_read_bytes;  /*!< number of bytes of commands actually read from
                             device */
  uint32_t d_write_bytes; /*!< number of bytes of commands actually written to
                             device */
  ...
  uint32_t in_calls_handled; /*!< number of ncoming/waiting calls passed to
                                dialplan */
  uint32_t in_pbx_fails;     /*!< number of start_pbx fails */

  uint32_t calls_answered[2]; /*!< number of outgoing and incoming/waiting calls
                                 answered */
  uint32_t
      calls_duration[2]; /*!< seconds of outgoing and incoming/waiting calls */
} pvt_stat_t;
...
#define SMS_INBOX_ARRAY_SIZE                                                   \
  ((SMS_INDEX_MAX + SMS_INBOX_ITEM_BITS - 1) / SMS_INBOX_ITEM_BITS)
...
typedef struct pvt {
  ...
  struct {
    struct at_queue_task *first;
    struct at_queue_task *last;
  } at_queue; /*!< queue for commands to modem */
  ...
  struct cpvt
      *last_dialed_cpvt; /*!< channel what last call successfully set ATDnum;
                            leave until ^ORIG received; need because real call
                            idx of dialing call unknown until ^ORIG */
...
#define DATA_READ_TIMEOUT 10000 /* 10 seconds */
  ...
  unsigned long
      channel_instance; /*!< number of channels created on this device */
  unsigned int
      rings; /*!< ring/ccwa  number distributed to at_response_clcc() */
  ...
  volatile unsigned int
      connected : 1; /*!< do we have an connection to a device */
  unsigned int
      initialized : 1; /*!< whether a service level connection exists or not */
  unsigned int gsm_registered : 1; /*!< do we have an registration to a GSM */
  unsigned int dialing;  /*!< HW state; true from ATD response OK until CEND or
                            CONN for this call idx */
  unsigned int ring : 1; /*!< HW state; true if has incoming call from first
                            RING until CEND or CONN */
  unsigned int cwaiting : 1; /*!< HW state; true if has incoming call waiting
                                from first CCWA until CEND or CONN for */
...
typedef struct public_state {
  ...
  volatile int unloading_flag; /* no need mutex or other locking for protect
                                  this variable because no concurent r/w and set
                                  non-0 atomically */
...
EXPORT_DECL int ready4voice_call(const struct pvt *pvt,
...
EXPORT_DECL struct pvt *find_device_ex(struct public_state *state,
                                       const char *name);
...
EXPORT_DECL struct pvt *
find_device_by_resource_ex(struct public_state *state, const char *resource,
                           int opts, const struct ast_channel *requestor,
                           int *exists);
EXPORT_DECL void pvt_dsp_setup(struct pvt *pvt, const char *id,
                               dc_dtmf_setting_t dtmf_new);

INLINE_DECL struct pvt *
find_device_by_resource(const char *resource, int opts,
