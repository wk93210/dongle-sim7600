...
static int lock_create(const char *lockfile) {
  ...
  fd =
      open(lockfile, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IRGRP | S_IROTH);
  if (fd >= 0) {
...
static void disconnect_dongle(struct pvt *pvt) {
  ...
  ast_copy_string(PVT_STATE(pvt, data_tty), CONF_UNIQ(pvt, data_tty),
...
#define SMS_INBOX_BIT(index)                                                   \
  ((sms_inbox_item_type)(1) << (index % SMS_INBOX_ITEM_BITS))
...
/* anybody wrote some to device before me, and not read results, clean pending
 * results here */
#/* */
...
static void *do_monitor_phone(void *data) {
  ...
  while (1) {
    ...
    while ((iovcnt = at_read_result_iov(dev, &read_result, &rb, iov)) > 0) {
      at_res =
          at_read_result_classification(&rb, iov[0].iov_len + iov[1].iov_len);
...
static inline int start_monitor(struct pvt *pvt) {
  if (ast_pthread_create_background(&pvt->monitor_thread, NULL,
                                    do_monitor_phone, pvt) < 0) {
...
static int pvt_discovery(struct pvt *pvt) {
  ...
  if (CONF_UNIQ(pvt, data_tty)[0] == 0 && CONF_UNIQ(pvt, audio_tty)[0] == 0) {
    ...
    ast_debug(3, "[%s] Trying ports discovery for%s%s%s%s\n", PVT_ID(pvt),
              imei[0] == 0 ? "" : " IMEI ", imei, imsi[0] == 0 ? "" : " IMSI ",
              imsi);
    ast_mutex_unlock(&pvt->lock);
    // sleep(10); // test
    ...
    if (resolved) {
      ast_copy_string(PVT_STATE(pvt, data_tty), data_tty,
                      sizeof(PVT_STATE(pvt, data_tty)));
      ...
      ast_verb(3, "[%s]%s%s%s%s found on data_tty=%s audio_tty=%s\n",
               PVT_ID(pvt), imei[0] == 0 ? "" : " IMEI ", imei,
               imsi[0] == 0 ? "" : " IMSI ", imsi, PVT_STATE(pvt, data_tty),
  ...
  while (state->unloading_flag == 0) {
    ...
    AST_RWLIST_TRAVERSE(&state->devices, pvt, entry) {
      ...
      if (pvt->restart_time == RESTATE_TIME_NOW &&
          pvt->desired_state != pvt->current_state) {
    ...
    /* actual device removal here for avoid long (discovery) time write lock on
     * device list in loop above */
...
static int discovery_restart(public_state_t *state) {
  ...
  if (state->discovery_thread != AST_PTHREADT_NULL) {
    ...
    /* Start a new monitor */
    if (ast_pthread_create_background(&state->discovery_thread, NULL,
                                      do_discovery, state) < 0) {
      ast_mutex_unlock(&state->discovery_lock);
...
#define TEST_BIT(dw_array, bitno)                                              \
  ((dw_array)[(bitno) >> 5] & 1 << ((bitno) & 31))
#/* */
EXPORT_DEF int pvt_get_pseudo_call_idx(const struct pvt *pvt) {
  ...
  AST_LIST_TRAVERSE(&pvt->chans, cpvt, entry) { SET_BIT(bits, cpvt->call_idx); }
...
#/* */
static int is_dial_possible2(const struct pvt *pvt, int opts,
                             const struct cpvt *ignore_cpvt) {
  struct cpvt *cpvt;
  int hold = 0;
  int active = 0;
...
EXPORT_DECL int pvt_enabled(const struct pvt *pvt) {
  return pvt->current_state == DEV_STATE_STARTED &&
         (pvt->desired_state == pvt->current_state ||
          pvt->restart_time == RESTATE_TIME_CONVENIENT);
}

#/* */
EXPORT_DEF int ready4voice_call(const struct pvt *pvt,
                                const struct cpvt *current_cpvt, int opts) {
  if (!pvt->connected || !pvt->initialized || !pvt->has_voice ||
      !pvt->gsm_registered || !pvt_enabled(pvt)) {
    return 0;
  }
...
#/* */
static int can_dial(struct pvt *pvt, int opts,
  ...
  if ((opts & CALL_FLAG_HOLD_OTHER) == CALL_FLAG_HOLD_OTHER &&
      channels_loop(pvt, requestor))
    return 0;
  return ready4voice_call(pvt, NULL, opts);
}

#/* return locked pvt or NULL */
EXPORT_DEF struct pvt *find_device_ex(struct public_state *state,
                                      const char *name) {
  struct pvt *pvt;
...
EXPORT_DEF struct pvt *
find_device_by_resource_ex(struct public_state *state, const char *resource,
                           int opts, const struct ast_channel *requestor,
                           int *exists) {
  int group;
  size_t i;
  size_t j;
  ...
  *exists = 0;
  ...
  if (((resource[0] == 'g') || (resource[0] == 'G')) &&
      ((resource[1] >= '0') && (resource[1] <= '9'))) {
    errno = 0;
  ...
  } else if (((resource[0] == 'r') || (resource[0] == 'R')) &&
             ((resource[1] >= '0') && (resource[1] <= '9'))) {
    errno = 0;
    group = (int)strtol(&resource[1], (char **)NULL, 10);
    if (errno != EINVAL) {
  ...
  } else if (((resource[0] == 'p') || (resource[0] == 'P')) &&
             resource[1] == ':') {
    /* Generate a list of all available devices */
    j = ITEMS_OF(round_robin);
    c = 0;
  ...
  } else if (((resource[0] == 's') || (resource[0] == 'S')) &&
             resource[1] == ':') {
    /* Generate a list of all available devices */
    j = ITEMS_OF(round_robin);
    c = 0;
  ...
  } else if (((resource[0] == 'i') || (resource[0] == 'I')) &&
             resource[1] == ':') {
    AST_RWLIST_TRAVERSE(&state->devices, pvt, entry) {
      ast_mutex_lock(&pvt->lock);
...
static const char *pvt_state_base(const struct pvt *pvt) {
  ...
  if (pvt->current_state == DEV_STATE_STOPPED &&
      pvt->desired_state == DEV_STATE_STOPPED)
    state = "Stopped";
...
EXPORT_DEF struct ast_str *pvt_str_state_ex(const struct pvt *pvt) {
  ...
  if (state)
    ...
    if (pvt->dialing || (PVT_STATE(pvt, chan_count[CALL_STATE_INIT]) +
                         PVT_STATE(pvt, chan_count[CALL_STATE_DIALING]) +
                         PVT_STATE(pvt, chan_count[CALL_STATE_ALERTING])) > 0)
...
EXPORT_DEF const char *GSM_regstate2str(int gsm_reg_status) {
  ...
  return enum2str_def(gsm_reg_status, gsm_states, ITEMS_OF(gsm_states),
                      "Unknown");
}
...
EXPORT_DEF const char *sys_mode2str(int sys_mode) {
  static const char *const sys_modes[] = {
      "No Service", "AMPS", "CDMA", "GSM/GPRS", "HDR", "WCDMA", "GPS",
  };
...
EXPORT_DEF const char *sys_submode2str(int sys_submode) {
  static const char *const sys_submodes[] = {
      "No service", "GSM",   "GPRS",  "EDGE",
      "WCDMA",      "HSDPA", "HSUPA", "HSDPA and HSUPA",
  };

  return enum2str_def(sys_submode, sys_submodes, ITEMS_OF(sys_submodes),
                      "Unknown");
}
...
static int pvt_time4restate(const struct pvt *pvt) {
  if (pvt->desired_state != pvt->current_state) {
    if (pvt->restart_time == RESTATE_TIME_NOW ||
        (PVT_NO_CHANS(pvt) && !pvt->outgoing_sms &&
         pvt->incoming_sms_index == -1U))
      return 1;
  }
  return 0;
...
static int pvt_reconfigure(struct pvt *pvt, const pvt_config_t *settings,
                           restate_time_t when) {
  int rv = 0;
...
#/* */
static int reload_config(public_state_t *state, int recofigure,
                         restate_time_t when, unsigned *reload_immediality) {
    ...
    if (strcasecmp(cat, "general") && strcasecmp(cat, "defaults")) {
      ...
      if (!err) {
        ...
        if (pvt) {
          if (!recofigure) {
            ast_log(LOG_ERROR,
                    "device %s already exists, duplicate in config file\n",
                    cat);
          } else {
            pvt->must_remove = 0;
...
static int public_state_init(struct public_state *state) {
  ...
  if (reload_config(state, 0, RESTATE_TIME_NOW, NULL) == 0) {
    ...
    ast_log(LOG_ERROR,
            "Errors reading config file " CONFIG_FILE ", Not loading module\n");
  }
